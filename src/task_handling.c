...
void create_new_task_box(struct CreateNewTaskBoxParams *params, int id) {
    ...
    gtk_box_append(GTK_BOX(content_box), name_and_important_box);
    gtk_box_append(GTK_BOX(content_box), task_date_label);
    gtk_box_append(GTK_BOX(content_box), task_desc_label);
    gtk_box_append(GTK_BOX(content_box), button_box);
    gtk_box_append(GTK_BOX(single_task_box), content_box);
...
void cancel_adding_new_task(GtkWidget *button, gpointer data){
    ...
    if (cancel_params!=0){
        gtk_widget_set_visible(cancel_params->existing_box, TRUE);
        add_task_box_parent = gtk_widget_get_parent(cancel_params->existing_box);
...
void add_new_task(GtkWidget *button, gpointer data) {
    if(!is_add_task_active) {
        ...
        GtkWidget *existing_box = gtk_widget_get_parent(gtk_widget_get_parent(gtk_widget_get_parent(button)));
        ...
        cancel_params.existing_box = 0;
        add_task_box = gtk_box_new(GTK_ORIENTATION_VERTICAL, 10);
        printf("%d", add_new_task_params->edit_mode);
        if (add_new_task_params->edit_mode==1) {
            GtkWidget *content_box = gtk_widget_get_parent(gtk_widget_get_parent(button));
            GtkWidget *first_content_box_child = gtk_widget_get_first_child(content_box);
            GtkWidget *second_content_box_child = gtk_widget_get_next_sibling(first_content_box_child);
            GtkWidget *third_content_box_child = gtk_widget_get_next_sibling(second_content_box_child);
            const char *task_name = gtk_label_get_label(GTK_LABEL(gtk_widget_get_first_child(first_content_box_child)));
            const char *task_desc = gtk_label_get_label(GTK_LABEL(third_content_box_child));
            const char *string_date = gtk_label_get_label(GTK_LABEL(second_content_box_child));
            gtk_label_set_label(GTK_LABEL(date_label), string_date);
            gtk_widget_set_visible(content_box, FALSE);
            gtk_box_append(GTK_BOX(existing_box), add_task_box);
            gtk_label_set_label(GTK_LABEL(label), "Edytowanie zadania");
            ...
            cancel_params.existing_box = content_box;
        }
        ...
        gtk_box_append(GTK_BOX(add_task_box), date_label);
        gtk_box_append(GTK_BOX(add_task_box), add_date_button);

        gtk_box_append(GTK_BOX(add_task_box), add_button);
        /* make tasks box inactive*/
        
        if (add_new_task_params->edit_mode==0) {
            gtk_widget_set_sensitive(tasks_box, false);
        }
        if (add_new_task_params->edit_mode==0) gtk_box_prepend(GTK_BOX(right_box), add_task_box);
        is_add_task_active= !is_add_task_active;
